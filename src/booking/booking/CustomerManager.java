...
public class CustomerManager {
    ...
    private final Map<booking.Customer, Integer> customers = new HashMap<>();
    ...
    public int getOrAddCustomerId(String firstName, String lastName) {
        booking.Customer customer = new booking.Customer(0, firstName, lastName); // temporary ID
        if (customers.containsKey(customer)) {
            return customers.get(customer);
        } else {
            int id = nextId++;
            customers.put(new booking.Customer(id, firstName, lastName), id);
            return id;
        }
    }
     ...
     */
    public booking.Customer getCustomer(String firstName, String lastName) {
